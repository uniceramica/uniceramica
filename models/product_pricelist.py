...
class ProductPricelist(models.Model):
    ...
    def _get_rules_sql2(self, products, date):
        self.ensure_one()
        categ_ids = set([])
    ...
    def _get_rules_sql(self, products, date):
        _select, _from, _where, _orderby, params = self._get_rules_sql2(
            products, date)
        brand_ids = [p.brand_id.id for p in products if p.brand_id]
        if not brand_ids:
            return _select, _from, _where, _orderby, params
        _where += (
            'AND (item.brand_id IS NULL '
            'OR item.brand_id = any(%s)) '
        )
        params.append(brand_ids)
        return _select, _from, _where, _orderby, params

    def _is_valid_rule2(self, rule, product, qty):
        self.ensure_one()
    ...
    def _is_valid_rule(self, rule, product, qty):
        is_valid = self._is_valid_rule2(rule, product, qty)
        if not is_valid:
            return is_valid
        if rule.brand_id:
